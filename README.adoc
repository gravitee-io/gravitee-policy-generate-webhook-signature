...
.^|headers [List]
^.^|-
|List of existing HTTP or Message headers to prepend their values to the body/message for generating the HMAC signature. These headers are read before `timestampValidity` generates its timestamp header, so `timestampValidity.targetTimestampHeader` must not be listed here.
^.^|
